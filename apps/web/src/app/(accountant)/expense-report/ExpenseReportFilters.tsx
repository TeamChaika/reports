...
const PRESETS = [
  ...
  { label: '30 дней', from: () => daysAgo(29), to: () => todayStr() },
]
...
export function ExpenseReportFilters({ from, to, est, establishments }: Props) {
  const router = useRouter()

  function apply(f: string, t: string, e: string) {
    const params = new URLSearchParams({ from: f, to: t })
    if (e) params.set('est', e)
    router.push(`/expense-report?${params.toString()}`)
  }
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap justify-end">
      ...
      <input
        type="date"
        value={from}
        onChange={e => { if (e.target.value) apply(e.target.value, to, est) }}
      ...
      <input
        type="date"
        value={to}
        onChange={e => { if (e.target.value) apply(from, e.target.value, est) }}
        className="input input--sm"
        style={{ width: 'auto' }}
        aria-label="По дату"
      />
